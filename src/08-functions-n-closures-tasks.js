...
function logger(func, logFunc) {
  return function withLogFunc(...args) {
    const loggingArgs = args.map((el) => JSON.stringify(el));

    logFunc(`${func.name}(${loggingArgs}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${loggingArgs}) ends`);
...
function partialUsingArguments(fn, ...args1) {
  return function combiner(...args2) {
